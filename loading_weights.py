...
def load_weights_from_hdf5_group(f, layers, reshape=False):
    ...
    for k, name in enumerate(layer_names):
        ...
        # customize weight
        if weight_values[0].shape != symbolic_weights[0].shape:
            weights = weight_values[0].copy()
            if name == 'conv1':
                new_weights = weights[:, :, 0, np.newaxis, :]
                new_weights = np.repeat(new_weights, new_weights.shape[0], axis=2)
                new_weights = np.expand_dims(new_weights, axis=-2)
            else:
                new_weights = weights[:, :, np.newaxis, ...]
                new_weights = np.repeat(new_weights, new_weights.shape[0], axis=2)
            weight_values = [new_weights.copy(), weight_values[1]]
